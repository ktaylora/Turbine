...
def _gdal_rasterize(shapefile_path, template=None, outfile=None, **kwargs):
    ...
    gdal.RasterizeLayer(
        dataset=target_ds, bands=kwargs["bands"], layer=layer, burn_values=1
    )
...
def get_project_region_extent(
    ...
    project_region = Vector(input=config_file, options=sql_options)
    project_region.crs = "+init=epsg:2163"
    return (
        project_region.to_geodataframe().to_crs("+init=epsg:4326").geometry.total_bounds
    )
...
def get_ned_elevation_raster(extent, res=250, **kwargs):
    ...
    if kwargs.get("output") is None:
        raise AttributeError(
            "output= argument is required by " + "elevation.clip() and cannot be None"
        )
    if os.path.isfile(kwargs.get("output")):
        logger.warning(
            "Warning : It looks like output="
            + kwargs.get("output")
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(elev):
        get_ned_elevation_raster(
            get_project_region_extent(),
            output="elevation.tif",
            res=250,
            max_download_tiles=500,
        )
        os.rename(
            os.path.expanduser("~") + "/.cache/elevation/SRTM3/elevation.tif",
    ...
    if not os.path.isfile(tri_3):
        tri_3 = ndimage_filter(
            elev.array, use_disc_caching=False, function=np.std, size=3
        )
    ...
    if not os.path.isfile(tri_11):
        tri_11 = ndimage_filter(
            elev.array, use_disc_caching=False, function=np.std, size=11
        )
    ...
    nrel_grid = Vector(
        "config.json", options={"table_name": "mboggie.project_region_buffered_1km"}
    ).to_geodataframe()

    nrel_grid = generate_h5_grid_geodataframe(filter_by_intersection=nrel_grid)
    ...
    nrel_grid = attribute_and_bootstrap_timeseries(
        gdf=nrel_grid, datasets=wind_products, n_bootstrap_replicates=30
    )

    # use a bilinear interpolation to downscale our attributed NREL grid to our 250 meter NED grid
    wind_rasters = [
        rasterize(nrel_grid, template=elev, field=f, dtype=np.dtype("float32"))
        for f in wind_products
    ]
    wind_rasters = [
        write_raster(
            array=wind_rasters[i], filename="raster/" + f + ".tif", template=elev
        )
